  ...
  <q-list separator bordered>
    ...
    <q-item v-for="(item, index) in items.slice(0, shownItems)" :key="index">
      ...
      <q-btn
        ...
        round
        size="sm"
        :color="getTaskColor('done')"
        :icon="getTaskIconAlt('done')"
        @click.native="$emit('occurrence-do', items.splice(index, 1)[0])"
      />
      <q-btn
        v-if="['done', 'failed', 'next'].includes(type) && isDue(item)"
        round
        size="sm"
      ...
      />
      <q-btn
        v-if="['done', 'skipped', 'next'].includes(type) && isDue(item)"
        round
        size="sm"
      ...
      />
      <q-btn
        v-if="['done', 'skipped', 'failed'].includes(type) && isDue(item)"
        round
        size="sm"
